@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex flex-col sm:flex-row items-center justify-between gap-4 w-full text-xs font-bold text-[#1A1A2E]">
        ...
        <div class="text-slate-600 font-bold">
            Menampilkan <span class="font-extrabold text-[#1A1A2E]">{{ $paginator->firstItem() }}</span> sampai <span class="font-extrabold text-[#1A1A2E]">{{ $paginator->lastItem() }}</span> dari <span class="font-extrabold text-[#1A1A2E]">{{ $paginator->total() }}</span> hasil
        </div>
        ...
        <div class="flex items-center gap-1.5 flex-wrap">
            ...
            @else
                <a href="{{ $paginator->previousPageUrl() }}" class="px-3 py-1.5 bg-[#FFFBEB] hover:bg-[#FFE156] border-2 border-[#1A1A2E] shadow-[2px_2px_0px_#1A1A2E] rounded-lg font-extrabold transition-all text-[#1A1A2E]">
                    ‹ Prev
                </a>
            @endif
            ...
            @if ($paginator->hasMorePages())
                <a href="{{ $paginator->nextPageUrl() }}" class="px-3 py-1.5 bg-[#FFFBEB] hover:bg-[#FFE156] border-2 border-[#1A1A2E] shadow-[2px_2px_0px_#1A1A2E] rounded-lg font-extrabold transition-all text-[#1A1A2E]">
                    Next ›
                </a>
            @else
                <span class="px-3 py-1.5 bg-slate-100 border-2 border-slate-300 text-slate-400 rounded-lg font-bold cursor-not-allowed">
