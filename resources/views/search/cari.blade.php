...
<div class="flex items-center gap-3">
    <a href="{{ route('search') }}" onclick="if (window.history.length > 1) { window.history.back(); return false; }" class="w-10 h-10 bg-white border-[3px] border-[#1A1A2E] rounded-full flex items-center justify-center font-bold shadow-[2px_2px_0px_#1A1A2E] shrink-0 hover:translate-y-[-2px] transition-transform cursor-pointer">
        &larr;
    </a>
    <h1 class="text-xl font-heading font-bold">Cari 🔍</h1>
</div>
@endsection
...
<x-card class="mb-5 bg-[#1A1A2E] text-white">
    <form action="{{ route('search.cari') }}" method="GET" class="flex gap-2">
        <input
            type="search"
            name="q"
            value="{{ $query ?? '' }}"
